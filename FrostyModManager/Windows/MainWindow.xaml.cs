...
using FrostyModManager.Converters;
...
public class ModResourceInfo
{
    ...
    private List<ModAction> mods = new();
    private List<int> addBundles = new();

    public ModResourceInfo(string n, string t)
    {
        Name = n;
        Type = t;
        nameHash = Utils.HashString(t + "/" + n);
    }
...
{
    private List<IFrostyMod> availableMods = new();
    private List<FrostyPack> packs = new();
    private FrostyPack selectedPack;
    private DirectoryInfo modsDir = new(Path.Combine("Mods", ProfilesLibrary.ProfileName));
    ...
    private void FrostyWindow_FrostyLoaded(object sender, EventArgs e)
    {
        ...
        Title = $"Frosty Mod Manager - 2.0.0 ({ProfilesLibrary.DisplayName})";

        Frosty.Core.App.PluginManager.Initialize();
        ...
        FrostyTaskWindow.Show("Loading Mods", "", (task) =>
        {
            ...
            Parallel.ForEach(modsDir.EnumerateFiles(), fi =>
            {
                ...
                {
                    int retCode = 0;
                    using (FileStream stream = new(fi.FullName, FileMode.Open, FileAccess.Read))
                ...
                task.TaskLogger.LogProgress(currentMod++ / (float)totalMods * 100d);
            });
            // load collections
            Parallel.ForEach(modsDir.EnumerateFiles(), fi =>
            {
        ...
        PropertyGroupDescription groupDescription = new("ModDetails.Category", null, StringComparison.OrdinalIgnoreCase);
        ...
        LoadedPluginsList.ItemsSource = Frosty.Core.App.PluginManager.Plugins;
    ...
    private void removeProfileButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (FrostyMessageBox.Show("Are you sure you want to delete this pack?", "Frosty Mod Manager", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
        {
            ...
            DirectoryInfo di = new(fs.BasePath + "ModData\\" + selectedPack.Name);
            if (di.Exists)
                di.Delete(true);
    ...
    private void launchButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        FrostyTaskWindow.Show("Launching", "", (task) =>
        {
            try
            {
                ...
                FrostyModExecutor modExecutor = new();
    ...
    private void FrostyWindow_Closing(object sender, CancelEventArgs e) => Config.Save(Frosty.Core.App.ConfigPath);
    ...
    private void availableModsList_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        FrostyModDescription modDescPanel = new() { Mod = mod };
    ...
    private void InstallMods(string[] filenames)
    {
        ...
        List<ImportErrorInfo> errors = new();
        ...
        FrostyTaskWindow.Show("Installing Mods", "", (task) =>
        {
                        ...
                        try
                        {
                            ...
                            foreach (CompressedFileInfo compressedFi in decompressor.EnumerateFiles())
                            {
                                ...
                                if (compressedFi.Extension == ".fbpack")
                                {
                                    ...
                                    FileInfo tempfile = new(tempdir + compressedFi.Filename);
            ...
            {
                FileInfo fi = new(Path.Combine(modsDir.FullName, collections[i]));
        ...
        PropertyGroupDescription groupDescription = new("ModDetails.Category");
        view.GroupDescriptions.Add(groupDescription);
    ...
    private void UpdateConflicts()
    {
        ...
        List<ModResourceInfo> totalResourceList = new();
                    ...
                    foreach (var mod in mods)
                    {
                        if (mod.NewFormat)
                        {
                            foreach (BaseModResource resource in mod.Resources)
                            {
                                ...
                                if (index == -1)
                                {
                                    ModResourceInfo resInfo = new(resourceName, resType);
        ...
        for (int i = 0; i < selectedPack.AppliedMods.Count; i++)
        {
            ...
            Binding primaryActionBinding = new("") { Converter = new ModPrimaryActionConverter(), ConverterParameter = appliedMod.Mod.Filename };
            Binding primaryTooltipBinding = new("") { Converter = new ModPrimaryActionTooltipConverter(), ConverterParameter = appliedMod.Mod.Filename };
            Binding secondaryActionBinding = new("") { Converter = new ModSecondaryActionConverter(), ConverterParameter = appliedMod.Mod.Filename };
            Binding secondaryTooltipBinding = new("") { Converter = new ModSecondaryActionTooltipConverter(), ConverterParameter = appliedMod.Mod.Filename };

            GridViewColumn gvc = new() { Header = appliedMod.Mod.ModDetails.Title, HeaderTemplate = conflictsListView.Resources["conflictsModHeaderTemplate"] as DataTemplate };
            DataTemplate dt = new(typeof(Grid));

            FrameworkElementFactory factory = new(typeof(StackPanel));
            ...
            FrameworkElementFactory img = new(typeof(Image));
            ...
            FrameworkElementFactory simg = new(typeof(Image));
    ...
    private void AddResourceAction(List<ModResourceInfo> totalResourceList, string modName, string resourceName, string resourceType, ModPrimaryActionType type)
    {
        ...
        if (index == -1)
        {
            ModResourceInfo resInfo = new(resourceName, resourceType);
            totalResourceList.Add(resInfo);
            index = totalResourceList.Count - 1;
        }
    ...
    {
        Setter setter = new(DockPanel.DockProperty, Dock.Top);
        ...
        Style style = new(typeof(ListBoxItem), FindResource(typeof(ListBoxItem)) as Style);
