...
public class FrostyObjectTypeConverter : IMultiValueConverter
{
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        {
            var attr = type.GetCustomAttribute<Frosty.Sdk.Attributes.DisplayNameAttribute>();
    ...
    public void AddRangeAndReplace(IEnumerable<Attribute> inAttrs)
    {
        ...
        foreach (var attr in inAttrs)
        {
            if (attr is EbxFieldMetaAttribute)
                continue;
            if (attr is EbxTypeMetaAttribute)
                continue;
...
public class FrostyPropertyGridItemData : INotifyPropertyChanged
{
    ...
    public bool IsModified
    {
        get
        {
            ...
            if (Value != null)
            {
                ...
                if (meta != null && meta.Flags.GetTypeEnum() == TypeFlags.TypeEnum.Struct)
    ...
    public bool HasItems
    {
        get
        {
            ...
            if (_value != null)
            {
                ...
                var meta = _value.GetType().GetCustomAttribute<EbxTypeMetaAttribute>();
    ...
    public bool FilterGuid(string guid, List<object> refObjects, bool doNotHideSubObjects = false)
    {
        ...
        foreach (var item in Children)
        {
            ...
            if (item.Value is PointerRef pr)
            {
                ...
                {
                    string fileGuidString = pr.External.PartitionGuid.ToString();
                    string classGuidString = pr.External.InstanceGuid.ToString();
    ...
    private void ProcessChildren()
    {
        if (_value != null)
        {
            ...
            if (tmpValue is BoxedValueRef boxedValue && boxedValue.Value != null)
            {
                tmpValue = _valueConverter.Convert(tmpValue, typeof(object), null, null);
                tmpDefValue = null;
                meta = new EbxTypeMetaAttribute(TypeFlags.TypeEnum.Struct);
            }

            if (meta != null && meta.Flags.GetTypeEnum() == TypeFlags.TypeEnum.Struct)
            {
                if (_valueConverter != null && _value is not BoxedValueRef)
                {
                    tmpValue = _valueConverter.Convert(tmpValue, typeof(object), null, null);
    ...
    private FrostyPropertyGridItemData[] ProcessClass(object value, object defValue, FrostyPropertyGridItemData parent)
    {
        ...
        foreach (PropertyInfo pi in pis)
        {
            ...
            string name = pi.Name;
            if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.DisplayNameAttribute>() != null)
                name = attributes.GetCustomAttribute<Frosty.Sdk.Attributes.DisplayNameAttribute>().Name;
            ...
            /*if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.DescriptionAttribute>() != null)
            ...
            /*if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.EditorAttribute>() != null)
                subItem.TypeEditor = attributes.GetCustomAttribute<Frosty.Sdk.Attributes.EditorAttribute>().EditorType;*/
...
public class FrostyPropertyGridItem : TreeViewItem
{
    ...
    private void CopyGuidMenuItem_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (pointerRef.Type == PointerRefType.Null)
            ...
            guidToCopy = pointerRef.External.InstanceGuid.ToString();
        else
        {
            ...
            if (item.IsEnabled && !item.IsReadOnly)
            {
                if (FrostyClipboard.Current.IsType(item.Value.GetType()))
                {
                    ...
                    if (pg.Asset != null)
                    {
                        ...
                        item.Value = FrostyClipboard.Current.GetData(pg.Asset, AssetManager.GetEbxAssetEntry(pg.Asset.PartitionGuid));
                    }
                    else
                    {
...
public class FrostyPropertyGrid : Control
{
    ...
    #region -- Asset --
    public static readonly DependencyProperty AssetProperty = DependencyProperty.Register("Asset", typeof(EbxPartition), typeof(FrostyPropertyGrid), new FrameworkPropertyMetadata(null, OnAssetChanged));
    public EbxPartition Asset
    {
        get => (EbxPartition)GetValue(AssetProperty);
    ...
    {
        FrostyPropertyGrid pg = sender as FrostyPropertyGrid;
        EbxPartition asset = args.NewValue as EbxPartition;
        pg.asset = asset;
    }
    #endregion
    ...
    public override void OnApplyTemplate()
    {
        ...
        if (asset != null)
            SetClass(asset.PrimaryInstance);
    ...
    protected virtual FrostyPropertyGridItemData[] ProcessClassWithCategories(object value)
    {
        ...
        foreach (PropertyInfo pi in pis)
        {
            ...
            if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.CategoryAttribute>() != null)
                category = attributes.GetCustomAttribute<Frosty.Sdk.Attributes.CategoryAttribute>().Name;
            ...
            if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.IsReadOnlyAttribute>() != null)
                subItem.IsReadOnly = true;
            /*if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.DescriptionAttribute>() != null)
                subItem.Description = attributes.GetCustomAttribute<Frosty.Sdk.Attributes.DescriptionAttribute>().Description;*/
            if (attributes.GetCustomAttribute<DependsOnAttribute>() != null)
                subItem.DependsOn = attributes.GetCustomAttribute<DependsOnAttribute>().Name;
            /*if (attributes.GetCustomAttribute<Frosty.Sdk.Attributes.EditorAttribute>() != null)
                subItem.TypeEditor = attributes.GetCustomAttribute<Frosty.Sdk.Attributes.EditorAttribute>().EditorType;*/
