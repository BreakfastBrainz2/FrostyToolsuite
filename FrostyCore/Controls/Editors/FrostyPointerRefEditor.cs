...
public class FrostyPointerRefControl : Control
{
    ...
    public string RefTooltip
    {
        get
        {
            ...
            if (pointerRef.Type == PointerRefType.Null)
                ...
                return pointerRef.External.InstanceGuid.ToString();
    ...
    private List<IEbxInstance> assignObjs = new();
    ...
    private void Popup_DropDownOpened(object sender, EventArgs e)
    {
        ...
        textBlock.Text = "Assign from " + ((isInternal) ? "self" : AssetManager.GetEbxAssetEntry(assignFileGuid).Name);
    ...
    private void Popup_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        else
        {
            ...
            editor.AddDependentObject(reference.PartitionGuid);
    ...
    private void OptionsButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        List<PointerRefClassType> types = new();
    ...
    private void AssignButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        else if (count == 1)
        {
            ...
            GetParentEditor().AddDependentObject(reference.PartitionGuid);
            Value = new PointerRef(reference);
        }
        else
        {
    ...
    protected void RefreshName()
    {
        ...
        {
            EbxAssetEntry entry = AssetManager.GetEbxAssetEntry(pointerRef.External.PartitionGuid);
            FrostyAssetEditor editor = GetParentEditor();
            EbxPartition asset = editor.GetDependentObject(pointerRef.External.PartitionGuid);

            if (entry != null)
            {
                if (asset == null || pointerRef.External.InstanceGuid == Guid.Empty || pointerRef.External.InstanceGuid == asset.PrimaryInstanceGuid)
                {
                ...
                else
                {
                    ...
                    dynamic obj = asset.GetObject(pointerRef.External.InstanceGuid);
                    if (obj == null)
                    {
                        asset = editor.RefreshDependentObject(pointerRef.External.PartitionGuid);
                        obj = asset.GetObject(pointerRef.External.InstanceGuid);
                    }
